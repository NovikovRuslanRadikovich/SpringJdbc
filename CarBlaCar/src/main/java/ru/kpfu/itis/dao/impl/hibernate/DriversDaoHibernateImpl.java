...
public class DriversDaoHibernateImpl implements DriversDao {
    ...
    private  final Logger logger = Logger.getLogger(DriversDaoHibernateImpl.class);
    ...
    public List<Driver> findTop9ByOrderByRatingDesc() {
        ...
        try{
            session = HibernateConnectionFactory.getSessionFactory().openSession();
            drivers = session.createCriteria(Driver.class).addOrder(Order.desc("rating")).setMaxResults(9).list();
        } catch(Exception e) {
              logger.error("error finding Top9 by Hibernate", e.getCause());
        } finally {
            if(session != null && session.isOpen()) {
                session.close();
            }
